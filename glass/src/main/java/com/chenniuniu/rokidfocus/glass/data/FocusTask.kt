...
data class FocusTask(
    ...
    val ev: Double get() = usd * p.coerceIn(0.0, 1.0)
    ...
    companion object {
        fun formatMinutes(raw: Int): String = formatMinutes(raw.toDouble())

        fun formatMinutes(raw: Double): String {
            val m = raw.coerceAtLeast(0.0)
            if (m > 0.0 && m < 1.0) {
                val s = if (m < 0.1) "%.2f".format(m) else "%.1f".format(m)
                return "${s.trimEnd('0').trimEnd('.')}m"
            }
            val whole = m.roundToInt()
            val h = whole / 60
            val mm = whole % 60
            return when {
                    ...
                    for (i in 0 until arr.length()) {
                        ...
                        add(
                            FocusTask(
                                ...
                                minutes = o.optDouble("minutes", 30.0).let { if (it.isNaN()) 30.0 else it.coerceAtLeast(0.0) },
                                p = o.optDouble("p", 1.0).let { if (it.isNaN()) 1.0 else it.coerceIn(0.0, 1.0) },
