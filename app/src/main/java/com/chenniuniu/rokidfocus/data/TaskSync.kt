...
object TaskSync {
    ...
    fun pull(baseUrl: String): SyncPayload {
        val conn = open(baseUrl, "GET")
        return try {
            val body = conn.inputStream.bufferedReader().readText()
            val obj = JSONObject(body)
            SyncPayload(
                tasks = FocusTask.fromJson(obj.optJSONArray("tasks")?.toString() ?: "[]"),
                opportunities = Opportunity.fromArray(obj.optJSONArray("opportunities")),
