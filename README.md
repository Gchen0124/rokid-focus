...
HUD **ddl** on a row is the locked deadline (`idealAt`). Laptop **Lock ETF as ddl** stamps *now + remaining minutes* (live ETF) onto each open row. Those times do **not** slide as the clock ticks.  
Bottom line shows locked **ddl** and live **ETF now**, vs **17:30**.
