...
## Opportunity hunting (daily, agent)

Standing contract: `memory/queue/1786973510012-opps-hunting.md`.

1. Any agent reads `memory/hunting/README.md` and runs the `PLAYBOOK.md` timeline (~25–40m).
2. Notion `funding sources` holds apply start/end + Next check. The dated report is `memory/hunting/reports/YYYY-MM-DD.md`.
3. Agent ends `ready`. Human applies. Do not mark the HUD row Done.
4. Monday (or a ≥6 day gap): add the weekly deep dig. Every other Monday: biweekly new-org pass.
