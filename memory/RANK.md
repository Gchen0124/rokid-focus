...
Glasses show **EV · time · title · ddl**, sized by EV. Slogan sits under the clock. Cutoff is **17:30**. Everything below is computed or talked — not drawn as extra chrome.
...
- HUD: `ev  minutes  title  ddl` (no rank numbers). Bottom: `need · ddl (ideal stack finish) · slack/over 5:30`. `ddl` is planned from the last list edit, not from this glance.
