...
3. `memory/DELEGATION.md` — if anything is assigned to the agent
4. `memory/RANK.md` — score formula
5. `memory/PROFILE.md` + `memory/PROJECTS.md` + `memory/PATTERNS.md` + `memory/DEPENDENCIES.md` if ranking or sequencing bets
...
| Delegation | `memory/DELEGATION.md`, `memory/queue/` | Work handed to an agent |
| Hunting | `memory/hunting/` + Notion `funding sources` | Daily opp radar; dates live in Notion |
| Glance UI | glasses `:glass` + `mock/index.html` | EV · time · title · ETF |
...
- HUD row: `$  time  title  ddl` — no rank numbers, no `$/h`. Agent-owned tasks stay off the waveguide. `ddl` is the ideal deadline, not overflow-from-now.
